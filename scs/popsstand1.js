...
bmbtz({
  ...
  try {
    ...
    for (let member of participants) {
      ...
      let name = member.name || member.notify || `[𝚩𝐔𝐒𝚻𝚫 𝚻𝚵𝐂𝚮] +${number}`;
      vcardData += `BEGIN:VCARD\nVERSION:3.0\nFN:${name}\nTEL;type=CELL;type=VOICE;waid=${number}:+${number}\nEND:VCARD\n`;
    }

    repondre(`A moment, *𝚩𝐔𝐒𝚻𝚫 𝚻𝚵𝐂𝚮* is compiling ${participants.length} contacts into a vcf...`);
